...
SAResults simulatedAnnealing(const vector<vector<int>>& distMatrix, 
                            double T0, double alpha, int NSteps);
...
shuffle(currentTour.begin(), currentTour.end(), gen);
...
uniform_int_distribution<int> dist_index(0, n-1);
...
int main(){
  ...
  // Measure execution time of the Simulated Annealing algorithm
  auto start = chrono::high_resolution_clock::now();

  // SA call
  SAResults results = simulatedAnnealing(distMatrix, T0, alpha, NSteps);
